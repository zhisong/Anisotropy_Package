...
*COMDECK COMMAX
      PARAMETER (LANZ=11,MANZ=LANZ, MDIF=1, LMAX=128)
      PARAMETER (LVANZ=11, MVANZ=LANZ, LVMAX=LMAX)
...
c            HV (NO) = - GEM33(NO)/T(NO) * (DSPPERAVG(NO)
c     >        + (GEM33AVG(NO) - GEM33(NO))/GEM33AVG(NO)/2.
c     >           * 2.*(DSPPERAVG(NO) 
c     >           - 2.*DSPPERAVG(NO)*PPERAVG(NO)/PPARAVG(NO)
c     >           + DSPPARAVG(NO)*PPERAVG(NO)**2/PPARAVG(NO)**2)*0)
C
...
c            HV(NO)= - GEM33(NO)/T(NO) * (DSPPARAVG(NO)
c     >        + (GEM33AVG(NO) - GEM33(NO))/GEM33AVG(NO)/2.
c     >           *(DSPPARAVG(NO) - DSPPERAVG(NO))*0)
...
C
      REAL     X(*), Y(NPDIM,2,*)
      INTEGER  IOP(30),IOP7(30), IP(30), NPOINT, IPN
      COMPLEX  EW
      CHARACTER*7   TITLE
      CHARACTER*(*) YNAME 
...
c      DATA IOP7 /190981,190991,191001,191011,191021,191031,191041/
*ENDIF
C
      XMIN = 0.0
      XMAX = X(NPOINT)
